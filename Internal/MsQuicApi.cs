...
internal sealed unsafe partial class MsQuicApi
{
    ...
    private MsQuicApi(QUIC_API_TABLE* apiTable)
    {
        ...
        fixed (byte* pAppName = "CSharp.Quic"u8)
        {
...
#pragma warning disable CA1810 // Initialize all static fields in 'MsQuicApi' when those fields are declared and remove the explicit static constructor
    ...
    static MsQuicApi()
    {
        ...
        string s_msQuicLibraryName = null;
        if (OperatingSystem.IsWindows())
        {
            s_msQuicLibraryName = "msquic.dll";
            ...
            path = System.IO.Path.Combine(path, s_msQuicLibraryName);
        ...
        {
            s_msQuicLibraryName = "libmsquic.so";
            // Non-Windows relies on the package being installed on the system and may include the version in its name
            loaded = NativeLibrary.TryLoad($"{s_msQuicLibraryName}.{s_minMsQuicVersion.Major}", typeof(MsQuicApi).Assembly, null, out msQuicHandle) ||
                     NativeLibrary.TryLoad(s_msQuicLibraryName, typeof(MsQuicApi).Assembly, null, out msQuicHandle);
        }

        if (!loaded)
        {
            // MsQuic library not loaded
            NotSupportedReason = $"Unable to load MsQuic library version '{s_msQuicLibraryName}.{s_minMsQuicVersion.Major}'.";
            if (NetEventSource.Log.IsEnabled())
            {
                NetEventSource.Info(null, NotSupportedReason);
            }
